...
- Build frontend: from `client` directory, `npm run build`
- Build backend server: from `backend\server` directory, `go build`. 
- Build backend pgn parser: from `backend\parser` directory, `go build`. 